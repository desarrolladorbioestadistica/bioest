...
//                localStorage.setItem("saveContinent", 0);
        ...
        self.div.find("#consultar-reportedian").on("click",function(e){
            columns=self.div.find("#columnsdd").val();
            console.log(columns);
            if(!columns || columns.length<2){
                $.notify({
                    message: "Debe seleccionar al menos dos campos"
                }       
                ,{
                    type: "warning"
                });
...
//        });
    ...
    self.filterDianToSearch=function(){
        ...
        $.ajax({
            ...
            if(response.status=='nosession'){
                ...
                if(response.status=='success'){
                    ...
                    var derivers = $.pivotUtilities.derivers;
                    var renderers = $.extend($.pivotUtilities.renderers,$.pivotUtilities.c3_renderers);
                    if(columns[0]=="id_oferente"){columns[0]="razon_social_oferente"}
                    if(columns[1]=="id_oferente"){columns[1]="razon_social_oferente"}
                    $("#output").pivotUI(response.data, {
                        renderers: renderers,
                        cols: [columns[0]], rows: [columns[1]],
                        rendererName: "Tabla",
                        rowOrder: "value_z_to_a", colOrder: "value_z_to_a",
                        rendererOptions: {
                            c3: { data: {colors: {
...
$(document).ready(function() {
    window.ReportesDian=new ReportesDian();
    ReportesDian.filterDianToSearch();
});